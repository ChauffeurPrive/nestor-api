...
import nestor_api.lib.k8s as k8s_lib


class TestK8s(TestCase):
    @patch("nestor_api.lib.k8s.write_and_deploy_configuration", autospec=True)
    @patch("nestor_api.lib.k8s.deploy_app_ingress", autospec=True)
    @patch("nestor_api.lib.k8s.K8sConfiguration", autospec=True)
    @patch("nestor_api.lib.k8s.has_web_process", autospec=True)
    @patch("nestor_api.lib.k8s.builders", autospec=True)
    ...
    def test_deploy_app_ingress(self, write_and_deploy_configuration_mock, builders_mock):
        ...
        k8s_lib.deploy_app_ingress(deployment_config, {})

        write_and_deploy_configuration_mock.assert_called_once_with("my-cluster", "ingress: app")

    def test_has_web_process_when_true(self):
        """Should return True."""
        deployment_config = {
            "processes": [
        ...
        result = k8s_lib.has_web_process(deployment_config)

        self.assertTrue(result)

    def test_has_web_process_when_false(self):
        """Should return False."""
        deployment_config = {
            "processes": [
                {"name": "worker", "is_cronjob": False},
    ...
    def test_has_web_process_when_web_is_a_cron(self):
        """Should not consider a "web" cron has a process."""
        deployment_config = {
            "processes": [
                {"name": "worker", "is_cronjob": False},
                {"name": "cronjob", "is_cronjob": True},
    ...
    @patch("nestor_api.lib.k8s.io", autospec=True)
    @patch("nestor_api.lib.k8s.cli", autospec=True)
